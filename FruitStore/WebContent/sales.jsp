...
<%	
	...
	if(comcodeS==null && comrateS==null){
 	...
 	for(FruitVO vo:list){ 
 		if(vo.getFruit_code()==comcode){ %>
 			<tr>  <td> <div> <%=vo.getFruit_code() %></div> <b class="sale"><%=(int)comrate %>%세일</b></td>  
 			<td><%=vo.getFruit_name() %></td>  
 	 		<td> <div class="mline"><%=dc.format(vo.getPrice()) %>원</div>  
 	 		<% comrate=(100.0-comrate)/100; %>
 	 		<b><%=dc.format(vo.getPrice()*comrate)  %>원</b></td> 
 	 		<td><%=vo.getQuantity() %></td>   
 	 		</tr>
 <%		}else{%>
 		<tr>  <td><%=vo.getFruit_code() %></td>  <td><%=vo.getFruit_name() %></td>  
 		<td><%=dc.format(vo.getPrice()) %>원</td> <td><%=vo.getQuantity() %></td>   
 		</tr>
<% 		}
 	} 	
%>
